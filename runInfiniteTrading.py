...
if __name__ == "__main__":
    ...
    stock_name = sys.argv[1]  # 주식 종목명
    balance_amount = int(sys.argv[2])  # 전체 잔액
    start_trade_date = sys.argv[3]  # 거래 시작일
    end_trade_date = sys.argv[4]  # 거래 종료일
    buy_times = int(sys.argv[5])  # 매수 회차
    buffer_trading_point = int(sys.argv[6])  # 버퍼 매매 포인트
    sell_point = int(sys.argv[7])  # 매도 포인트
    buffer_buy_divide = int(sys.argv[8]) # 버퍼 매수 분할 수치
    buffer_sell_divide = int(sys.argv[9]) # 버퍼 매도 분할 수치
    
    # print(f"stock_name: {stock_name}, balance_amount: {balance_amount}")
